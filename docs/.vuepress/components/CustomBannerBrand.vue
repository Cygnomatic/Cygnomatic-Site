    ...
    <div class="banner-brand__content">
      ...
      <ul class="social-links" v-if="socialLinks.length > 0">
        <li class="social-item" v-for="(item, index) in socialLinks" :key="index">
          <Xicons v-if="!item.local" :icon="item.icon" :link="item.link" :style="{ color: item.color }" target="_blank" />
          <Xicons v-else :link="item.link" :style="{ color: item.color }" target="_blank">
            <template #icon>
              <component :is="localIcons[item.icon]" />
            </template>
          </Xicons>
        </li>
      </ul>
